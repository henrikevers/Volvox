...
{
    public class Instr_MeshCrop : Instr_BaseReporting
    {
        #region - Instruction Initialization
        /// <summary>
        /// Initialize Instruction Input Variables
        /// </summary>
        private List<Mesh> insV_Mesh { get; set; }
        ...
        /// <param name="M"></param>
        /// <param name="I"></param>
        public Instr_MeshCrop(List<Mesh> M, Boolean I)
        {
             insV_Mesh = M;
        ...
        */
        ...
        {
            get { return GUIDs.GuidsRelease4.Instruction.Instr_MeshCrop; }
        }
        ...
        {
            List<Mesh> nl = new List<Mesh>(insV_Mesh);
            Boolean nI = insV_InsideBool;

            Instr_MeshCrop ni = new Instr_MeshCrop(nl, nI);
            ni.GlobalCloud = null;
            ni.NewCloud = null;
            ni.CloudPieces = null;
            ni.DictPieces = null;
            ...
            Parallel.ForEach(rangePartitioner, po, (rng, loopState) =>
            {
                ...
                if (GlobalCloud.UserDictionary.Count > 0)
                {
                    ...
                    for (int i = rng.Item1; i < rng.Item2; i++)
                    {
                        ...
                        if (insV_InsideBool)
                        {
                            for (int j = 0; j <= insV_Mesh.Count - 1; j += 1)
                            {
                                PointCloudItem GlobalCloudItem = GlobalCloud[i];
                                if (insV_Mesh[j].IsPointInside(GlobalCloudItem.Location, Rhino.RhinoDoc.ActiveDoc.ModelAbsoluteTolerance, false))
                                {
                        ...
                        else
                        {
                            ...
                            for (int j = 0; j <= insV_Mesh.Count - 1; j += 1)
                            {
                                CropBools.Add(insV_Mesh[j].IsPointInside(GlobalCloudItem.Location, Rhino.RhinoDoc.ActiveDoc.ModelAbsoluteTolerance, false));
                            }
